module SocialSnippet
  module SnippetFinder
    class SnippetFinderBase
      attr_reader :repo_manager

      # Constructor
      #
      # @param [SocialSnippet::RepositoryManager]
      ...
      def each_snip_tags(src, context, base_tag)
        TagParser.find_snip_tags(src).each do |tag_info|
          ...
          snippet = repo_manager.get_snippet(context, t)
      ...
      def resolve_tag_repo_ref!(tag)
        repo = repo_manager.find_repository_by_tag(tag)

        # not found
        return if repo.nil?

        if tag.has_ref? === false || tag.ref != repo.get_latest_version(tag.ref)
          if repo.has_versions?
            tag.set_ref repo.get_latest_version(tag.ref)
          else
            tag.set_ref repo.get_short_commit_id
          end
        end
      end
